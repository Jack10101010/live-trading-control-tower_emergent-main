...
def build_snapshot(
    *,
    ...
    # Fall back to the ARMED account fingerprint when no runtime identity sample
    # exists this cycle: the armed binding is still node-observed truth.
    arming_summary = safe_arming(arm_runtime, mode)
    if identity["available"] is not True and arming_summary["account_fingerprint"]:
        identity = {**identity, "fingerprint": arming_summary["account_fingerprint"],
                    "server": arming_summary.get("server")}
