...
describe('structural isolation of fixture fleet data', () => {
  ...
  it('M-EVENTS-1: no ordinary source reads WORLD events directly', () => {
    // `/api/events` was the LAST endpoint that merged fixture rows with runtime
    // rows into one chronological collection. Two components also read
    // `world.events` straight from the fixture, bypassing the endpoint entirely.
    const offenders: string[] = [];
    for (const f of allSources(SRC)) {
      if (/world\.events\b/.test(stripComments(readFileSync(f, 'utf8')))) offenders.push(rel(f));
    }
    expect(offenders, offenders.join('\n')).toEqual([]);
  });
